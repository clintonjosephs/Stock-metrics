...
import Data from '../__mocks__/MockMetrics.js';
...
describe('Test all synchronous actions', () => {
    ...
    const dataPush =  metricsDataReducer(metricsData, Actions.loadStockMetrics(Data));
    ...
    it('loads all company details',() => {
        const { details } = metricsDataReducer(dataPush, Actions.loadCompanyDetails(MockDetails.details));
        expect(details.length).toBe(1);
        expect(details[0].companyName).toEqual('Apple Inc.');
    });

    it('loads all company income statement', () => {
        const { statement } = metricsDataReducer(dataPush, Actions.loadCompanyStatement(MockDetails.statement));
        expect(statement.length).toBe(5);
        expect(statement[0].symbol).toEqual('AAPL');
    })

    it('test filter stock companies', () => {
        const { dataWorker } = metricsDataReducer(dataPush, Actions.filterCompanies('ford'));
    ...
    it('test that company details is reset', () => {
        const { details, statement } = metricsDataReducer(dataPush, Actions.resetDetails);
